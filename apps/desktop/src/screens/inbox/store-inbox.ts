...
// (the section Task) is kept as is. A rule with a judge statement (slice 26,
// ADR 0012) needs the Judge's answer per Thread: the Thread falls through to
// the next rule until the answer is here, the missing answers are asked for
// in one batched request through the content transport, and the stream is
