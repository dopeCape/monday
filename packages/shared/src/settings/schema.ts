...
export const settingsSchema = {
  ...
  "inbox.memory_window": setting({
    type: z.int().min(100).max(100000),
    default: 1500,
    scope: "device",
    section: "appearance",
    group: "Inbox",
    tier: "advanced",
    label: "Threads held in memory",
    help: "How many of the newest Threads each list (the Inbox, a folder, a Group) holds in memory at once. Scrolling near the end reads this many more, so every Thread stays in reach. Fewer uses less memory for large Accounts. Per device.",
  }),
  "inbox.memory_grow_rows": setting({
    ...
    group: "Inbox",
    tier: "advanced",
    label: "Read more before the end",
    help: "How many rows before the end of what a list holds scrolling (or j and k) reads the next Threads. Per device.",
  }),
  "inbox.memory_lookups": setting({
    ...
    group: "Inbox",
    tier: "advanced",
    label: "Other Threads kept at hand",
    help: "How many Threads opened from outside the list (a search result, a link, a notification, the agent) stay in memory after they are closed. An open Thread always stays. Per device.",
  }),
  ...
  "routing.page.recent_shown": setting({
    ...
    group: "Groups",
    tier: "advanced",
    label: "Recently routed on the Routing page",
    help: "How many of the newest routed threads the Routing page lists, each with why it went where it did; 0 hides the list.",
  }),
  ...
  "routing.group_icons": setting({
    ...
    tier: "advanced",
    label: "Group icons",
    help: "The icon a Group shows in the nav, by a word in its name: users-three, user-plus, receipt, handshake, github-logo, microphone, calendar, archive, check-circle, airplane, scales, lifebuoy, newspaper, briefcase, house, heart, tag, folder, git-branch, code, bell, warning, users, book-open, shopping-bag, shield, megaphone, chat-circle, rocket, currency-dollar, graduation-cap, chart-line, truck. A Group no word matches shows the fallback icon.",
  }),
  "routing.group_icon_fallback": setting({
    type: z.string().min(1),
    default: "folder",
    scope: "global",
    section: "routing",
    group: "Groups",
    tier: "advanced",
    label: "Group icon fallback",
    help: "The icon a Group shows when no word in its name matches Group icons. One of the icon names listed there.",
  }),
  ...
  "workflows.page.runs_shown": setting({
  ...
  "workflows.page.step_details": setting({
    ...
    group: "Defaults",
    tier: "advanced",
    label: "Show each step's fields",
    help: "Each step card on the Workflows page lists its fields (channel, message, folder) under its summary. Off shows the summary only.",
  }),
  ...
  "notifications.new_mail": setting({
    type: z.boolean(),
    default: true,
    scope: "device",
    section: "accounts",
    ...
    visibleWhen: { key: "notifications.enabled", truthy: true },
    label: "New mail",
    help: "A desktop notification when unread mail reaches the Inbox of any account, while the window is elsewhere or shows another account.",
  }),
  "notifications.new_mail_bulk": setting({
    type: z.boolean(),
    default: false,
    scope: "device",
    section: "accounts",
    group: "Notifications",
    tier: "more",
    visibleWhen: { key: "notifications.new_mail", truthy: true },
    label: "Newsletters too",
    help: "Also tell me about bulk mail: newsletters, promotions and automated senders.",
  }),
  "notifications.new_mail_recent_minutes": setting({
    type: z.int().min(1).max(1440),
    default: 10,
    scope: "device",
    section: "accounts",
    group: "Notifications",
    tier: "advanced",
    label: "New mail window",
    help: "Minutes a message may be old and still count as new. Mail caught up after the app was closed, or pulled in a first sync, is older and stays quiet.",
  }),
  ...
  "sync.warm_workspaces": setting({
    ...
    group: "Sync",
    tier: "advanced",
    label: "Keep every account ready",
    help: "Open every account's saved mail at start and keep it syncing, so switching accounts is instant and new mail is already there. Off opens an account only when you switch to it.",
  }),
  ...
  "strings.inbox.group_empty": str(
    "routing",
    "An empty Group: new mail is sorted in as it arrives",
    "No mail in {group} yet. New mail is sorted into it as it arrives; the mail already here moves when you sort it.",
  ),
  "strings.inbox.group_sort": str(
    "routing",
    "Button: sort existing mail into a Group",
    "Sort existing mail into {group}",
  ),
  "strings.inbox.group_sort_prompt": str(
    "routing",
    "What the button asks the Agent",
    "Sort the mail already here into {group}: show me what would move first.",
  ),
  ...
  /* The Workflows page redone: the list, the locked state, the flow of cards. */
  "strings.workflows.status.on": str("workflows", "Workflow status: on", "On"),
  "strings.workflows.status.off": str("workflows", "Workflow status: switched off", "Paused"),
  "strings.workflows.status.locked": str(
    "workflows",
    "Workflow status: AI level too low",
    "Locked",
  ),
  "strings.workflows.status.failing": str("workflows", "Workflow status: failing", "Failing"),
  "strings.workflows.runs_count": str("workflows", "Recent runs count", "{n} recent runs"),
  "strings.workflows.runs_one": str("workflows", "One recent run", "1 recent run"),
  "strings.workflows.list_label": str("workflows", "Workflow list label", "Your workflows"),
  "strings.workflows.asked_for": str("workflows", "The sentence label", "What you asked for"),
  "strings.workflows.flow_title": str("workflows", "The flow heading", "How it runs"),
  "strings.workflows.rename": str("workflows", "Rename button", "Rename"),
  "strings.workflows.rename_save": str("workflows", "Rename: save", "Save"),
  "strings.workflows.rename_cancel": str("workflows", "Rename: cancel", "Cancel"),
  "strings.workflows.change": str("workflows", "Change with monday button", "Change with monday"),
  "strings.workflows.hide_source": str("workflows", "Hide the Source", "Hide source"),
  "strings.workflows.showing_run": str(
    "workflows",
    "The flow shows one Run",
    "Showing what happened on {subject}, {when}",
  ),
  ...
  "strings.workflows.locked.body": str(
    "workflows",
    "Locked: body with Workflows",
    "At {level} nothing runs on its own. Your {n} workflows are kept exactly as they are, paused, and start again when you raise the level.",
  ),
  "strings.workflows.locked.body_empty": str(
    "workflows",
    "Locked: body with no Workflows",
    "At {level} nothing runs on its own. Raise the level, describe what should happen, and the agent writes the workflow and shows you every step first.",
  ),
  "strings.workflows.locked.kept": str(
    "workflows",
    "Locked: the kept Workflows heading",
    "Kept while paused",
  ),
  "strings.workflows.locked.benefits": setting({
    type: z.array(z.string()),
    default: [
      "Describe a workflow in a sentence and the agent writes the steps",
      "Runs on new mail, on a schedule, or when a thread goes quiet",
      "Posts to Slack, Notion, Drive or Discord only after you approve",
    ],
    scope: "global",
    section: "workflows",
    label: "Locked: what Workflows bring",
    help: "The lines the Workflows page shows while the AI level keeps Workflows paused.",
  }),
  "strings.workflows.locked.example": setting({
    type: z.array(z.string()),
    default: [
      "When an invoice arrives in Finance",
      "Read the vendor and the amount",
      "Save it to Drive",
    ],
    scope: "global",
    section: "workflows",
    label: "Locked: the example flow",
    help: "The trigger and steps of the faded example on the locked Workflows page, in order.",
  }),
  "strings.workflows.flow.when": str("workflows", "Flow: trigger label", "When"),
  "strings.workflows.flow.step": str("workflows", "Flow: step number", "Step {n}"),
  "strings.workflows.flow.trigger.arrival": str("workflows", "Trigger: arrival", "Mail arrives"),
  "strings.workflows.flow.trigger.arrival_in": str(
    "workflows",
    "Trigger: arrival in a Group",
    "Mail arrives in {group}",
  ),
  "strings.workflows.flow.trigger.arrival_from": str(
    "workflows",
    "Trigger: arrival from someone",
    "Mail arrives from {who}",
  ),
  "strings.workflows.flow.trigger.schedule": str(
    "workflows",
    "Trigger: schedule",
    "On a schedule: {when}",
  ),
  "strings.workflows.flow.trigger.manual": str(
    "workflows",
    "Trigger: manual",
    "Only when you run it",
  ),
  "strings.workflows.flow.trigger.silence": str(
    "workflows",
    "Trigger: silence",
    "No reply from you for {days} days",
  ),
  "strings.workflows.flow.trigger.silence_in": str(
    "workflows",
    "Trigger: silence in a Group",
    "No reply from you for {days} days in {group}",
  ),
  "strings.workflows.flow.trigger.event": str(
    "workflows",
    "Trigger: something happens to a thread",
    "A thread is {event}",
  ),
  "strings.workflows.flow.fact.from": str("workflows", "Trigger fact: from", "From"),
  "strings.workflows.flow.fact.subject": str("workflows", "Trigger fact: subject", "Subject has"),
  "strings.workflows.flow.fact.list": str("workflows", "Trigger fact: list", "Mailing list"),
  "strings.workflows.flow.fact.header": str("workflows", "Trigger fact: header", "Header"),
  "strings.workflows.flow.fact.attachment": str(
    "workflows",
    "Trigger fact: attachment",
    "With an attachment",
  ),
  "strings.workflows.flow.fact.value": str("workflows", "Trigger fact: tag or Group", "Which"),
  "strings.workflows.flow.fact.cron": str("workflows", "Trigger fact: cron", "Cron, UTC"),
  "strings.workflows.flow.fact.group": str("workflows", "Trigger fact: Group", "Group"),
  "strings.workflows.flow.judge": str(
    "workflows",
    "Flow: a judged test",
    "The judge agrees: {statement}",
  ),
  "strings.workflows.flow.threshold": str(
    "workflows",
    "Flow: judge threshold",
    "{pct}% sure or more",
  ),
  "strings.workflows.flow.kind.tag": str("workflows", "Step kind: tag", "Tag"),
  "strings.workflows.flow.kind.move": str("workflows", "Step kind: move", "Move"),
  "strings.workflows.flow.kind.archive": str("workflows", "Step kind: archive", "Archive"),
  "strings.workflows.flow.kind.snooze": str("workflows", "Step kind: snooze", "Snooze"),
  "strings.workflows.flow.kind.draft_reply": str("workflows", "Step kind: draft", "Draft a reply"),
  "strings.workflows.flow.kind.send": str("workflows", "Step kind: send", "Send"),
  "strings.workflows.flow.kind.notify": str("workflows", "Step kind: notify", "Notify you"),
  "strings.workflows.flow.kind.wait": str("workflows", "Step kind: wait", "Wait"),
  "strings.workflows.flow.kind.condition": str("workflows", "Step kind: condition", "Check"),
  "strings.workflows.flow.kind.slack": str("workflows", "Step kind: Slack", "Post to Slack"),
  "strings.workflows.flow.kind.notion": str("workflows", "Step kind: Notion", "Add to Notion"),
  "strings.workflows.flow.kind.drive": str("workflows", "Step kind: Drive", "Save to Drive"),
  "strings.workflows.flow.kind.discord": str("workflows", "Step kind: Discord", "Post to Discord"),
  "strings.workflows.flow.kind.webhook": str("workflows", "Step kind: webhook", "Call a webhook"),
  "strings.workflows.flow.kind.mcp": str("workflows", "Step kind: MCP tool", "Use a tool"),
  "strings.workflows.flow.kind.agentic": str("workflows", "Step kind: agentic", "Agent step"),
  "strings.workflows.flow.summary.tag_add": str("workflows", "Step: add tags", "Adds {add}"),
  "strings.workflows.flow.summary.tag_remove": str(
    "workflows",
    "Step: remove tags",
    "Removes {remove}",
  ),
  "strings.workflows.flow.summary.tag_both": str(
    "workflows",
    "Step: add and remove tags",
    "Adds {add} and removes {remove}",
  ),
  "strings.workflows.flow.summary.move": str(
    "workflows",
    "Step: move",
    "Moves the thread to {group}",
  ),
  "strings.workflows.flow.summary.move_out": str(
    "workflows",
    "Step: move out of every Group",
    "Takes the thread out of every Group",
  ),
  "strings.workflows.flow.summary.archive": str(
    "workflows",
    "Step: archive",
    "Archives the thread",
  ),
  "strings.workflows.flow.summary.snooze": str(
    "workflows",
    "Step: snooze",
    "Snoozes the thread for {hours} h",
  ),
  "strings.workflows.flow.summary.draft_voice": str(
    "workflows",
    "Step: draft in voice",
    "Writes a reply in your voice and leaves it in Drafts",
  ),
  "strings.workflows.flow.summary.draft_template": str(
    "workflows",
    "Step: draft from a template",
    "Writes a reply from a template and leaves it in Drafts",
  ),
  "strings.workflows.flow.summary.send": str(
    "workflows",
    "Step: send",
    "Sends the draft from {step}",
  ),
  "strings.workflows.flow.summary.notify": str(
    "workflows",
    "Step: notify",
    "Sends you a notification",
  ),
  "strings.workflows.flow.summary.wait": str(
    "workflows",
    "Step: wait",
    "Waits {hours} h before going on",
  ),
  "strings.workflows.flow.summary.condition": str(
    "workflows",
    "Step: condition",
    "Goes on only if {test}",
  ),
  "strings.workflows.flow.summary.slack": str(
    "workflows",
    "Step: Slack",
    "Posts to {channel} on Slack",
  ),
  "strings.workflows.flow.summary.notion": str(
    "workflows",
    "Step: Notion",
    "Adds a row to the {database} database",
  ),
  "strings.workflows.flow.summary.drive": str(
    "workflows",
    "Step: Drive",
    "Saves the attachment to {folder}",
  ),
  "strings.workflows.flow.summary.discord": str(
    "workflows",
    "Step: Discord",
    "Posts to {channel} on Discord",
  ),
  "strings.workflows.flow.summary.webhook": str(
    "workflows",
    "Step: webhook",
    "Sends a {method} request to {host}",
  ),
  "strings.workflows.flow.summary.mcp": str(
    "workflows",
    "Step: MCP tool",
    "Calls {tool} on {server}",
  ),
  "strings.workflows.flow.summary.agentic": str(
    "workflows",
    "Step: agentic",
    "The agent follows your instructions",
  ),
  "strings.workflows.flow.op.contains": str(
    "workflows",
    "Test: contains",
    "{left} contains {value}",
  ),
  "strings.workflows.flow.op.equals": str("workflows", "Test: equals", "{left} is {value}"),
  "strings.workflows.flow.op.matches": str("workflows", "Test: matches", "{left} matches {value}"),
  "strings.workflows.flow.op.exists": str("workflows", "Test: exists", "{left} is present"),
  "strings.workflows.flow.op.not_contains": str(
    "workflows",
    "Test: does not contain",
    "{left} does not contain {value}",
  ),
  "strings.workflows.flow.op.judged": str(
    "workflows",
    "Test: judged",
    "the judge agrees: {statement}",
  ),
  "strings.workflows.flow.hole.step": str(
    "workflows",
    "Template hole: a step's field",
    "{field} from {step}",
  ),
  "strings.workflows.flow.hole.subject": str("workflows", "Template hole: subject", "the subject"),
  "strings.workflows.flow.hole.from": str("workflows", "Template hole: sender", "the sender"),
  "strings.workflows.flow.arg.text": str("workflows", "Step field: message", "Message"),
  "strings.workflows.flow.arg.template": str("workflows", "Step field: template", "Template"),
  "strings.workflows.flow.arg.instructions": str(
    "workflows",
    "Step field: instructions",
    "Instructions",
  ),
  "strings.workflows.flow.arg.tools": str("workflows", "Step field: tools", "May use"),
  "strings.workflows.flow.arg.outputs": str("workflows", "Step field: outputs", "Reports"),
  "strings.workflows.flow.arg.budget": str("workflows", "Step field: budget", "Budget"),
  "strings.workflows.flow.arg.file_name": str("workflows", "Step field: file name", "File name"),
  "strings.workflows.flow.arg.body": str("workflows", "Step field: body", "Body"),
  "strings.workflows.flow.arg.args": str("workflows", "Step field: arguments", "Arguments"),
  "strings.workflows.flow.tools_all": str("workflows", "Step field: every tool", "Every tool"),
  "strings.workflows.flow.budget.calls": str("workflows", "Budget: calls", "{n} tool calls"),
  "strings.workflows.flow.budget.tokens": str("workflows", "Budget: tokens", "{n} tokens"),
  "strings.workflows.flow.budget.minutes": str("workflows", "Budget: minutes", "{n} min"),
  "strings.workflows.flow.tier.ask": str("workflows", "Step tier: asks", "Asks first"),
  "strings.workflows.flow.tier.undo": str(
    "workflows",
    "Step tier: reversible",
    "Applies with Undo",
  ),
  "strings.workflows.flow.tier.read": str("workflows", "Step tier: read-only", "Changes nothing"),
  "strings.workflows.flow.tier.standing": str(
    "workflows",
    "Step tier: standing approval",
    "Runs on your standing approval",
  ),
  "strings.workflows.flow.branch.yes": str("workflows", "Branch: yes", "If yes"),
  "strings.workflows.flow.branch.no_stop": str(
    "workflows",
    "Branch: no, the Run ends",
    "If not, the run ends here",
  ),
  "strings.workflows.flow.branch.no_skip": str(
    "workflows",
    "Branch: no, skip the next Step",
    "If not, skips {step}",
  ),
  "strings.workflows.flow.on_failure.stop": str(
    "workflows",
    "Failure policy: stop",
    "If this fails, the run stops",
  ),
  "strings.workflows.flow.on_failure.skip": str(
    "workflows",
    "Failure policy: skip",
    "If this fails, the run goes on",
  ),
  "strings.workflows.flow.on_failure.notify": str(
    "workflows",
    "Failure policy: notify",
    "If this fails, you are told and the run goes on",
  ),
  "strings.workflows.flow.change.added": str("workflows", "Change: new", "New"),
  "strings.workflows.flow.change.changed": str("workflows", "Change: changed", "Changed"),
  "strings.workflows.flow.change.removed": str("workflows", "Change: removed", "Removed"),
  "strings.workflows.flow.diff": str(
    "workflows",
    "Change summary",
    "{added} new, {changed} changed, {removed} removed",
  ),
  "strings.workflows.flow.diff_none": str(
    "workflows",
    "Change summary: steps unchanged",
    "The steps stay the same",
  ),
  "strings.workflows.flow.run.done": str("workflows", "Step outcome: done", "Done"),
  "strings.workflows.flow.run.failed": str("workflows", "Step outcome: failed", "Failed"),
  "strings.workflows.flow.run.waiting": str(
    "workflows",
    "Step outcome: waiting",
    "Waiting for you",
  ),
  "strings.workflows.flow.run.skipped": str("workflows", "Step outcome: skipped", "Skipped"),
  "strings.workflows.flow.run.running": str("workflows", "Step outcome: running", "Running"),
  "strings.workflows.flow.run.not_reached": str(
    "workflows",
  ...
  "strings.routing.subtitle": str(
    ...
    "Groups and Sections, each with a rule in plain words. Routing sorts new mail by them and learns from every thread you move.",
  ),
  ...
  /* The Routing page redone: the locked state, the overview, the tabs, the Group cards, why. */
  "strings.routing.locked.title": str("routing", "Locked: heading", "Sorting is paused"),
  "strings.routing.locked.lede": str(
    "routing",
    "Locked: what to do",
    "Increase the AI level to unlock Routing.",
  ),
  "strings.routing.locked.body": str(
    "routing",
    "Locked: body with Groups",
    "At {level} new mail is not sorted into your {n} Groups and nothing is judged. The Groups, their rules, Sections and custom actions are all kept, and moving mail into a Group by hand still works.",
  ),
  "strings.routing.locked.body_empty": str(
    "routing",
    "Locked: body with no Groups",
    "At {level} new mail is not sorted. Raise the level and describe a Group in a sentence; the agent writes the rule and shows what would move before anything does.",
  ),
  "strings.routing.locked.benefits": setting({
    type: z.array(z.string()),
    default: [
      "New mail sorted into Groups by rules you write in a sentence",
      "A Needs a decision queue whenever a rule is unsure",
      "Rules that learn from every thread you move",
    ],
    scope: "global",
    section: "routing",
    label: "Locked: what Routing brings",
    help: "The lines the Routing page shows while the AI level keeps sorting paused.",
  }),
  "strings.routing.tab.groups": str("routing", "Tab: Groups", "Groups"),
  "strings.routing.tab.sections": str("routing", "Tab: Sections", "Sections and judgments"),
  "strings.routing.tab.actions": str("routing", "Tab: custom actions", "Custom actions"),
  "strings.routing.stat.groups": str("routing", "Overview: Groups", "{n} Groups"),
  "strings.routing.stat.sections": str("routing", "Overview: Sections", "{n} Sections"),
  "strings.routing.stat.decisions": str("routing", "Overview: to decide", "{n} to decide"),
  "strings.routing.stat.sorting_on": str(
    "routing",
    "Overview: sorting on arrival",
    "Sorting new mail as it arrives",
  ),
  "strings.routing.stat.sorting_manual": str(
    "routing",
    "Overview: sorting when asked",
    "Sorting only when you re-run",
  ),
  "strings.routing.stat.sorting_paused": str(
    "routing",
    "Overview: sorting paused",
    "Sorting paused",
  ),
  "strings.routing.rule_label": str("routing", "Group card: rule label", "Rule"),
  "strings.routing.always_label": str("routing", "Group card: always label", "Always"),
  "strings.routing.pred.domain": str("routing", "Always: a domain", "Anyone at {value}"),
  "strings.routing.pred.sender": str("routing", "Always: a sender", "From {value}"),
  "strings.routing.pred.subject": str("routing", "Always: a subject", "Subject has {value}"),
  "strings.routing.pred.list": str("routing", "Always: a list", "List {value}"),
  "strings.routing.pred.attachment": str("routing", "Always: an attachment", "With an attachment"),
  "strings.routing.threshold": str(
    "routing",
    "Group card: threshold",
    "Asks you below {pct}% sure",
  ),
  "strings.routing.threads": str("routing", "Group card: thread count", "{n} threads"),
  "strings.routing.learned": str(
    "routing",
    "Group card: corrections",
    "Learned from {n} of your corrections",
  ),
  "strings.routing.learned_one": str(
    "routing",
    "Group card: one correction",
    "Learned from 1 of your corrections",
  ),
  "strings.routing.learned_show": str("routing", "Group card: show corrections", "Show"),
  "strings.routing.learned_hide": str("routing", "Group card: hide corrections", "Hide"),
  "strings.routing.open": str("routing", "Group card: open in the inbox", "Open"),
  "strings.routing.why.predicate": str("routing", "Why: the rule's facts", "Matched {fact}"),
  "strings.routing.why.model": str(
    "routing",
    "Why: the rule sentence",
    "Read the rule, {pct}% sure",
  ),
  "strings.routing.why.user": str("routing", "Why: moved by you", "You put it here"),
  "strings.routing.why.rule": str("routing", "Why: the rule", "Sorted by the rule"),
  "strings.routing.recent_empty": str("routing", "Recently routed empty", "Nothing sorted yet."),
  "strings.routing.candidate": str("routing", "Needs a decision: a candidate", "{name} {pct}%"),
  ...
  "strings.agent.preview_workflow.create": str(
    "ai",
    "Workflow card: create",
    "New workflow {name}. It starts switched off.",
  ),
  "strings.agent.preview_workflow.update": str(
    "ai",
    "Workflow card: change",
    "Changes {name} to version {version}. Earlier runs keep theirs.",
  ),
  "strings.agent.preview_workflow.enable": str("ai", "Workflow card: turn on", "Turns on {name}"),
  "strings.agent.preview_workflow.disable": str(
    "ai",
    "Workflow card: turn off",
    "Turns off {name}",
  ),
  "strings.agent.preview_workflow.renamed": str(
    "ai",
    "Workflow card: renamed",
    "Renamed from {name}",
  ),
  "strings.agent.preview_workflow.trigger": str(
    "ai",
    "Workflow card: trigger changed",
    "The trigger changes",
  ),
  ...
  /* The locked state a page shows when the AI level keeps its feature paused. */
  "strings.ai.lock.raise": str("ai", "Locked page: raise the level", "Raise to {level}"),
  "strings.ai.lock.confirm": str("ai", "Locked page: confirm the level", "Turn on {level}"),
  "strings.ai.lock.confirm_note": str(
    "ai",
    "Locked page: what raising does",
    "Routing, background Briefs and Workflows start. Anything that leaves your mailbox still asks first, and you can lower the level at any time.",
  ),
  "strings.ai.lock.cancel": str("ai", "Locked page: cancel", "Not now"),
  "strings.ai.lock.settings": str("ai", "Locked page: open Settings", "See the AI levels"),
  "strings.ai.lock.pinned": str(
    "ai",
    "Locked page: level set in the file",
    "The AI level is set in monday.toml, so change it there.",
  ),
  "strings.ai.lock.gets": str("ai", "Locked page: what you get", "What you get"),
  "strings.ai.lock.paused": str("ai", "Locked page: paused tag", "Paused"),
  ...
  "strings.notifications.new_mail_many": str(
    "accounts",
    "Notification title for several new emails",
    "{n} new emails",
  ),
