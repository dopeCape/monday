...
export const settingsSchema = {
  ...
  "sync.gmail_batch_parts": setting({
    ...
    group: "Sync",
    tier: "advanced",
    label: "Gmail batch size",
    help: "Requests monday puts in one Gmail batch. Gmail runs them at once and refuses too many in flight for one user; monday halves the batch when that happens and grows it back to this size.",
  }),
  "sync.body_concurrency": setting({
    type: z.int().min(1).max(20),
    default: 5,
    scope: "global",
    section: "accounts",
    group: "Sync",
    tier: "advanced",
    label: "Bodies at once",
    help: "Email bodies fetched at the same time during a sync. More is faster until the provider refuses; the quota pacing still applies to each one.",
  }),
  ...
  /* Compose windows and the writing tools (docs/spec/inbox.md, "Composing several messages at once") */
  "compose.close_behavior": setting({
    type: z.enum(["minimize", "close"]),
    default: "minimize",
    scope: "global",
    section: "accounts",
    group: "Compose",
    label: "Closing a message",
    help: "What Esc, the close button and a click outside do to a message with content: minimize it into the dock, or close it. Either way the Draft is kept; Discard is the only way to throw one away.",
  }),
  "compose.dock_position": setting({
    type: z.enum(["bottom-right", "bottom-left", "bottom-full"]),
    default: "bottom-right",
    scope: "global",
    section: "accounts",
    group: "Compose",
    label: "Dock position",
    help: "Where minimized messages sit: along the bottom right, the bottom left, or across the whole bottom edge.",
  }),
  "compose.dock_max_visible": setting({
    type: z.int().min(1).max(12),
    default: 3,
    scope: "global",
    section: "accounts",
    group: "Compose",
    label: "Minimized messages shown",
    help: "How many minimized messages show as chips before the rest fold into a +N list.",
  }),
  "compose.new_while_open": setting({
    type: z.enum(["minimize", "stack"]),
    default: "minimize",
    scope: "global",
    section: "accounts",
    group: "Compose",
    label: "New message while one is open",
    help: "Minimize the open message into the dock, or keep it open beside the new one.",
  }),
  "compose.window_style": setting({
    type: z.enum(["sheet", "docked", "fullscreen"]),
    default: "sheet",
    scope: "global",
    section: "accounts",
    group: "Compose",
    label: "Compose window",
    help: "A floating sheet over the inbox, a window docked at the bottom edge that leaves the inbox usable, or the full window.",
  }),
  "compose.restore_docked_on_launch": setting({
    type: z.boolean(),
    default: true,
    scope: "global",
    section: "accounts",
    group: "Compose",
    label: "Restore minimized messages on launch",
    help: "Messages left minimized when monday closed reappear in the dock. They are Drafts either way.",
  }),
  "compose.toolbar": setting({
    type: z.boolean(),
    default: true,
    scope: "global",
    section: "accounts",
    ...
    help: "Show bold, italic, link, lists, quote and clear formatting above the message. The keyboard shortcuts work either way.",
  }),
  "compose.assist": setting({
    type: z.boolean(),
    default: true,
    scope: "global",
    section: "accounts",
    ...
    help: "Offer rewrite, fix grammar, translate, continue writing and a free instruction in the composer when the AI level is on and a runtime is set up. Each result is a suggestion you accept or reject, metered like any Task.",
  }),
  "compose.assist_voice": setting({
    type: z.boolean(),
    default: true,
    scope: "global",
    section: "accounts",
    group: "Compose",
    label: "Assist in my voice",
    help: "When the Voice profile is on, the writing assist writes the way you do.",
  }),
  "compose.translate_default": setting({
    ...
    help: "The language Translate offers first. The menu takes any other language by name.",
  }),
  "compose.assist_prompt": setting({
    type: z.string().min(1).max(4000),
    default:
      "You edit one email the user is writing. Apply the requested change to the given text and answer with the new text only: no preface, no quotes around it, no subject line, no sign-off unless the text already has one. Keep names, facts, numbers, links and the language unless asked to change them. Keep paragraphs as blank-line separated plain text.",
    scope: "global",
    section: "accounts",
    group: "Compose",
    control: "sentence",
    tier: "advanced",
    label: "Assist prompt",
    help: "What the model is told for every writing assist in the composer.",
  }),
  "compose.assist_max_chars": setting({
    type: z.int().min(200).max(100_000),
    default: 20_000,
    scope: "global",
    section: "accounts",
    group: "Compose",
    tier: "advanced",
    label: "Assist text limit",
    help: "The most characters of a message the writing assist sends to the model at once.",
  }),
  ...
  "reader.load_remote_images": setting({
    ...
    help: "Fetch images a message links from the web. Tracking pixels are removed either way (see Tracker hosts). Off blocks every remote image until you ask, so senders cannot tell you opened a message.",
  }),
  "reader.attachment_preview": setting({
    type: z.boolean(),
    default: true,
    scope: "global",
    section: "routing",
    group: "Reading",
    label: "Preview attachments",
    help: "Clicking an attachment opens it in a window inside monday: images, PDFs, Word documents, spreadsheets, CSV, text and code, audio, video, zip contents and attached emails. Off downloads it instead.",
  }),
  "reader.preview_max_mb": setting({
    type: z.int().min(1).max(500),
    default: 50,
    scope: "global",
    section: "routing",
    ...
    help: "Megabytes an attachment may be for monday to preview it; a larger one offers Download only.",
  }),
  "reader.preview_max_rows": setting({
    type: z.int().min(50).max(100_000),
    default: 2000,
    scope: "global",
    section: "routing",
    group: "Reading",
    ...
    help: "Rows shown per sheet or CSV in the attachment preview.",
  }),
  "reader.tracker_hosts": setting({
    ...
    tier: "advanced",
    label: "Tracker hosts",
    help: "Images from these hosts (and their subdomains) are open trackers and never load. Tiny or hidden remote images are removed too, whatever their host.",
  }),
  ...
  "ai.composer.markdown": setting({
    ...
    help: "Render the Agent's answers as Markdown: lists, tables, code with a copy button, links. Off shows the plain text.",
  }),
  "ai.composer.fold_activity": setting({
    type: z.boolean(),
    default: true,
    scope: "global",
    section: "ai",
    group: "Sessions",
    tier: "advanced",
    label: "Fold the steps",
    help: "Fold a turn's read-only steps into one line once the answer starts. A card that asks or can be undone always stays open.",
  }),
  "ai.composer.input_history": setting({
    type: z.boolean(),
    default: true,
    scope: "global",
    section: "ai",
    ...
    tier: "advanced",
    label: "Input history",
    help: "Up in an empty composer recalls what you sent earlier in the Session; Down steps back.",
  }),
  "ai.composer.max_rows": setting({
    type: z.int().min(1).max(20),
    default: 8,
    scope: "global",
    section: "ai",
    ...
    help: "The most lines the composer grows to before it scrolls. Shift-Enter starts a new line.",
  }),
  "ai.composer.commands": setting({
    type: z.record(z.string().regex(/^[a-z][a-z0-9-]*$/), z.string().min(1).max(500)),
    default: {
      draft: "Draft a reply to ",
      summarize: "Summarize ",
      find: "Find ",
      organize: "Organize ",
      "plan-week":
        "Plan my week: read my calendar for this week and my mail for tasks, deadlines and meetings people asked for, then propose focus blocks, those meetings and prep time as one calendar draft for me to review.",
    },
    scope: "global",
    section: "ai",
    group: "Sessions",
    ...
    help: "Typing / in the composer lists these; picking one starts the message with its words. /new always starts a new conversation.",
  }),
  "ai.composer.mentions": setting({
    type: z.boolean(),
    default: true,
    scope: "global",
    section: "ai",
    ...
    tier: "advanced",
    label: "Mentions",
    help: "Typing @ in the composer lists Threads, Groups, Sections and people to point the Agent at.",
  }),
  "ai.composer.mention_threads": setting({
    type: z.int().min(0).max(100),
    default: 20,
    scope: "global",
    section: "ai",
    group: "Sessions",
    tier: "advanced",
    label: "Threads to mention",
    help: "How many of the newest Threads, and the people in them, the @ list offers.",
  }),
  "ai.composer.timestamps": setting({
    type: z.boolean(),
    default: true,
    scope: "global",
    section: "ai",
    ...
    tier: "advanced",
    label: "Turn times",
    help: "Show when each turn was sent and each answer began, on hover.",
  }),
  "ai.composer.elapsed_after_seconds": setting({
    type: z.int().min(0).max(600),
    default: 3,
    scope: "global",
    section: "ai",
    group: "Sessions",
    tier: "advanced",
    label: "Show time spent after",
    help: "Seconds a turn works before its Working line counts the time. 0 never counts.",
  }),
  ...
  "notifications.calendar": setting({
    type: z.boolean(),
    default: true,
    scope: "global",
    section: "accounts",
    ...
    label: "Before each event",
    help: "A desktop notification before each event you have not declined. An event's own reminders replace the time below.",
  }),
  "notifications.calendar_lead_minutes": setting({
    ...
    help: "Minutes before an event to remind you, unless the event sets its own reminders.",
  }),
  ...
  "calendar.day_start_hour": setting({
    ...
    help: "The hour your working day starts: earlier hours are shaded, and the Day and Week views open scrolled to it. Every hour of the day stays in the view.",
  }),
  "calendar.day_end_hour": setting({
    ...
    help: "The hour your working day ends: later hours are shaded in the Day and Week views.",
  }),
  ...
  "calendar.shown": setting({
    type: z.record(z.string(), z.record(z.string(), z.boolean())),
    default: {},
    scope: "global",
    section: "appearance",
    group: "Calendar",
    ...
    label: "Calendars in each workspace",
    help: "Which connected calendars each workspace shows, your other accounts' and those shared with you included. Every workspace shows every calendar unless you turn one off here or in the Calendar's sidebar.",
  }),
  "calendar.agent_draft_focus": setting({
    type: z.boolean(),
    default: true,
    scope: "global",
    section: "appearance",
    group: "Calendar",
    tier: "primary",
    label: "Open the Calendar on the Agent's drafts",
    help: "When the Agent proposes calendar changes, switch to the Calendar on the days they touch, with the changes shown over it. Off, the draft waits in the conversation until you open it.",
  }),
  "calendar.draft_card_rows": setting({
    type: z.int().min(1).max(50),
    default: 6,
    scope: "global",
    section: "appearance",
    group: "Calendar",
    tier: "advanced",
    label: "Changes listed on a draft card",
    help: "How many of a calendar draft's changes its card in the conversation lists before it says how many more there are.",
  }),
  "calendar.guest_suggestions_max": setting({
    type: z.int().min(1).max(20),
    default: 8,
    scope: "global",
    section: "appearance",
    ...
    help: "How many people the guest field suggests as you type, from your mail and earlier Events.",
  }),
  "calendar.zone_suggestions_max": setting({
    ...
    group: "Calendar",
    tier: "advanced",
    label: "Time zones listed",
    help: "How many time zones the editor's time zone field lists at once while you type.",
  }),
  "calendar.time_zone": setting({
    ...
    help: "The time zone the Agent reads your working hours in when it looks for free time, such as Europe/Dublin. Empty uses UTC.",
  }),
  "calendar.dates_month_first": setting({
    type: z.boolean(),
    default: false,
    scope: "global",
    section: "appearance",
    ...
    help: "Read a typed date such as 3/10 as March 10 rather than 3 October, in the palette's jump to date.",
  }),
  "calendar.default_view": setting({
    type: z.enum(["day", "week", "month", "agenda"]),
    default: "week",
    scope: "global",
    section: "appearance",
    group: "Calendar",
    label: "Calendar opens on",
    help: "The view the Calendar shows when you open it: Day, Week, Month or Agenda.",
  }),
  "calendar.work_days": setting({
    type: z.array(z.enum(["sun", "mon", "tue", "wed", "thu", "fri", "sat"])),
    default: ["mon", "tue", "wed", "thu", "fri"],
    scope: "global",
    section: "appearance",
    group: "Calendar",
    label: "Working days",
    help: "The days your working hours apply to; the others are shaded in the Day and Week views.",
  }),
  "calendar.show_weekends": setting({
    type: z.boolean(),
    default: true,
    scope: "global",
    section: "appearance",
    group: "Calendar",
    label: "Show weekends",
    help: "Off, the Week view shows your working days only.",
  }),
  "calendar.week_numbers": setting({
    type: z.boolean(),
    default: false,
    scope: "global",
    section: "appearance",
    group: "Calendar",
    label: "Week numbers",
    help: "Show the ISO week number in the Week and Month views and the mini month.",
  }),
  "calendar.secondary_time_zone": setting({
    type: z.string().max(64),
    default: "",
    scope: "global",
    section: "appearance",
    ...
    help: "A time zone such as America/New_York shown as a second column of hours in the Day and Week views. Empty shows none.",
  }),
  "calendar.hour_height": setting({
    type: z.int().min(24).max(160),
    default: 48,
    scope: "device",
    section: "appearance",
    group: "Calendar",
    label: "Hour height",
    help: "Pixels one hour takes in the Day and Week views on this computer.",
  }),
  "calendar.snap_minutes": setting({
    ...
    group: "Calendar",
    label: "Snap to",
    help: "Minutes an Event snaps to when you drag to make, move or resize it.",
  }),
  "calendar.drag_threshold_px": setting({
    type: z.int().min(1).max(24),
    default: 4,
    scope: "device",
    section: "appearance",
    ...
    tier: "advanced",
    label: "Drag distance",
    help: "Pixels the pointer moves before a press on the Calendar becomes a drag rather than a click.",
  }),
  "calendar.month_events_max": setting({
    type: z.int().min(1).max(12),
    default: 3,
    scope: "global",
    section: "appearance",
    group: "Calendar",
    label: "Events per day in Month",
    help: "How many Events a day in the Month view lists before it says how many more there are.",
  }),
  "calendar.agenda_days": setting({
    type: z.int().min(1).max(90),
    default: 14,
    scope: "global",
    section: "appearance",
    group: "Calendar",
    label: "Agenda length",
    help: "Days the Agenda view lists at a time.",
  }),
  "calendar.other_accounts": setting({
    type: z.boolean(),
    default: true,
    scope: "global",
    section: "appearance",
    group: "Calendar",
    label: "Every account's calendars",
    help: "Show the calendars of your other accounts beside this one's, each in its own colour. Off shows this account's only.",
  }),
  "calendar.mini_month": setting({
    type: z.boolean(),
    default: true,
    scope: "global",
    section: "appearance",
    group: "Calendar",
    label: "Mini month",
    help: "The small month beside the Calendar, for picking a day.",
  }),
  "calendar.sidebar": setting({
    type: z.boolean(),
    default: true,
    scope: "device",
    section: "appearance",
    ...
    help: "The column beside the Calendar with the mini month, today and your calendars. The sidebar button in the Calendar's header flips it.",
  }),
  "calendar.colors": setting({
    type: z.record(z.string(), z.string().max(32)),
    default: {},
    scope: "global",
    section: "appearance",
    group: "Calendar",
    ...
    help: "The colour you picked for a calendar, by its id: a palette colour (tag-1 to tag-5) or a hex value. Pick one from the swatch beside a calendar.",
  }),
  "calendar.reminder_choices": setting({
    type: z.array(z.int().min(0).max(40320)).max(12),
    default: [0, 5, 10, 15, 30, 60, 1440],
    scope: "global",
    section: "appearance",
    group: "Calendar",
    tier: "advanced",
    label: "Reminder choices",
    help: "The minutes before an Event the editor offers as reminders.",
  }),
  ...
  "accounts.status_poll_seconds": setting({
    ...
    help: "Seconds between checks of every Account's sync state, which is how the switcher and the sign-in notice learn that an Account needs attention. Per device.",
  }),
  ...
  "strings.switcher.signin": str(
    "accounts",
    "Workspace switcher: the provider refused the sign-in",
    "Sign in again",
  ),
  "strings.reauth.banner": str(
    "accounts",
    "Main screen: an Account's sign-in was refused",
    "{provider} stopped accepting monday's sign-in for {address}. New mail won't arrive until you sign in again.",
  ),
  "strings.reauth.banner_many": str(
    "accounts",
    "Main screen: several Accounts' sign-ins were refused",
    "{count} accounts need you to sign in again before new mail arrives.",
  ),
  "strings.reauth.action": str("accounts", "Main screen: sign in again button", "Sign in again"),
  "strings.reauth.dismiss": str(
    "accounts",
    "Main screen: hide the sign-in notice for now",
    "Later",
  ),
  ...
  "strings.viewer.close": str("routing", "Attachment viewer: Close the attachment", "Close"),
  "strings.viewer.download": str(
    "routing",
    "Attachment viewer: Download the attachment",
    "Download",
  ),
  "strings.viewer.previous": str(
    "routing",
    "Attachment viewer: Previous attachment",
    "Previous attachment",
  ),
  "strings.viewer.next": str("routing", "Attachment viewer: Next attachment", "Next attachment"),
  "strings.viewer.loading": str(
    "routing",
    "Attachment viewer: Opening an attachment",
    "Opening {name}",
  ),
  "strings.viewer.no_preview": str(
    "routing",
    "Attachment viewer: No preview for this type",
    "monday can't show {name} here. Download it to open it with another app.",
  ),
  "strings.viewer.too_large": str(
    "routing",
    "Attachment viewer: Attachment over the preview limit",
    "{name} is {size}, over the preview limit. Download it to open it.",
  ),
  "strings.viewer.failed": str(
    "routing",
    "Attachment viewer: Attachment preview failed",
    "{name} could not be shown: {message}",
  ),
  "strings.viewer.rows_more": str(
    "routing",
    "Attachment viewer: Rows cut off in a sheet or CSV",
    "Showing the first {n} of {total} rows.",
  ),
  "strings.viewer.page_of": str("routing", "Attachment viewer: PDF page count", "{total} pages"),
  "strings.viewer.zip_files": str("routing", "Attachment viewer: Files in a zip", "{n} files"),
  "strings.viewer.from": str("routing", "Attachment viewer: Attached email: sender", "From"),
  "strings.viewer.to": str("routing", "Attachment viewer: Attached email: recipients", "To"),
  "strings.viewer.date": str("routing", "Attachment viewer: Attached email: date", "Date"),
  "strings.viewer.zoom_in": str("routing", "Attachment viewer: Zoom in on an image", "Zoom in"),
  "strings.viewer.zoom_out": str("routing", "Attachment viewer: Zoom out of an image", "Zoom out"),
  "strings.viewer.label": str(
    "routing",
    "Attachment viewer: Attachment viewer name for assistive tech",
    "Attachment {name}",
  ),
  ...
  "strings.agent.send": str("ai", "Composer: send", "Send"),
  "strings.agent.stop": str("ai", "Composer: stop the turn", "Stop"),
  "strings.agent.stopped": str("ai", "Stopped turn line", "Stopped"),
  "strings.agent.steps_one": str("ai", "Folded steps: one", "1 step"),
  "strings.agent.steps_many": str("ai", "Folded steps: many", "{n} steps"),
  "strings.agent.copy": str("ai", "Copy an answer", "Copy"),
  "strings.agent.copied": str("ai", "Copied an answer or code", "Copied"),
  ...
  "strings.agent.continue": str("ai", "Continue a stopped turn", "Continue"),
  "strings.agent.continue_prompt": str(
    "ai",
    "The turn Continue sends",
    "Continue where you stopped.",
  ),
  "strings.agent.command_new": str("ai", "Slash command: /new", "Start a new conversation"),
  "strings.agent.mention.threads": str("ai", "Mention list: Threads", "Threads"),
  "strings.agent.mention.groups": str("ai", "Mention list: Groups", "Groups"),
  "strings.agent.mention.sections": str("ai", "Mention list: Sections", "Sections"),
  "strings.agent.mention.people": str("ai", "Mention list: people", "People"),
  "strings.agent.mention.back": str("ai", "Mention list: back to the kinds", "Back"),
  "strings.agent.asks.always": str(
    "ai",
    "Approval card note: always-ask",
    "Nothing leaves your mailbox until you approve.",
  ),
  "strings.agent.asks.reversible": str(
    "ai",
    "Approval card note: reversible",
    "Applies with Undo.",
  ),
  ...
  "strings.calendar.close": str("accounts", "Calendar: close", "Close"),
  "strings.calendar.color": str("accounts", "Calendar list: colour button", "Colour"),
  "strings.calendar.color_reset": str("accounts", "Calendar list: colour reset", "Its own colour"),
  "strings.calendar.copy_title": str("accounts", "Duplicate: the copy's title", "{title} (copy)"),
  "strings.calendar.delete": str("accounts", "Event: delete", "Delete"),
  "strings.calendar.delete_failed": str(
    "accounts",
    "Deleting an Event failed",
    "Could not delete the Event: {message}",
  ),
  "strings.calendar.update_failed": str(
    "accounts",
    "Changing an Event failed",
    "Could not change the Event: {message}",
  ),
  "strings.calendar.visible_failed": str(
    "accounts",
    "Showing a calendar failed",
    "Could not change that calendar: {message}",
  ),
  "strings.calendar.retry_failed": str(
    "accounts",
    "Reading a calendar again failed",
    "Could not read the calendar again: {message}",
  ),
  "strings.calendar.duplicate": str("accounts", "Event: duplicate", "Duplicate"),
  "strings.calendar.edit": str("accounts", "Event: edit", "Edit"),
  "strings.calendar.edit_event": str("accounts", "Editor title: editing", "Edit Event"),
  "strings.calendar.new_event_long": str("accounts", "New Event button, long", "New Event"),
  "strings.calendar.loading": str("accounts", "Calendar list: loading", "Reading your calendars."),
  "strings.calendar.no_calendars": str(
    "accounts",
    "Calendar list: none",
    "No calendars yet. They appear after the first sync.",
  ),
  "strings.calendar.no_events_body": str(
    "accounts",
    "Agenda empty, body",
    "Nothing in the next {n} days.",
  ),
  "strings.calendar.more": str("accounts", "Month: more Events", "{n} more"),
  "strings.calendar.open_day": str("accounts", "Day heading: open the Day view", "Open this day"),
  "strings.calendar.previous_month": str("accounts", "Mini month: previous", "Previous month"),
  "strings.calendar.next_month": str("accounts", "Mini month: next", "Next month"),
  "strings.calendar.sidebar": str("accounts", "Calendar sidebar label", "Calendar sidebar"),
  "strings.calendar.sidebar_toggle": str(
    "accounts",
    "Header: sidebar button",
    "Show or hide the sidebar",
  ),
  "strings.calendar.to": str("accounts", "Between two times", "to"),
  "strings.calendar.untitled": str("accounts", "An Event without a title", "(No title)"),
  "strings.calendar.waiting": str(
    "accounts",
    "Sidebar: invites waiting",
    "Waiting for your answer",
  ),
  "strings.calendar.week_short": str("accounts", "A week number", "W{n}"),
  "strings.calendar.toast.added": str("accounts", "Toast: Event added", "Event added"),
  "strings.calendar.toast.updated": str("accounts", "Toast: Event changed", "Event changed"),
  "strings.calendar.toast.deleted": str("accounts", "Toast: Event deleted", "Event deleted"),
  "strings.calendar.detail.guests": str("accounts", "Detail: guests heading", "{n} guests"),
  "strings.calendar.detail.in_zone": str(
    "accounts",
    "Detail: the Event's own zone",
    "{from} to {to} in {zone}",
  ),
  "strings.calendar.detail.yes_count": str("accounts", "Detail: guests who said yes", "{n} yes"),
  "strings.calendar.detail.maybe_count": str(
    "accounts",
    "Detail: guests who said maybe",
    "{n} maybe",
  ),
  "strings.calendar.detail.no_count": str("accounts", "Detail: guests who said no", "{n} no"),
  "strings.calendar.detail.waiting_count": str(
    "accounts",
    "Detail: guests yet to answer",
    "{n} awaiting",
  ),
  "strings.calendar.detail.optional": str("accounts", "Detail: optional guest", "optional"),
  "strings.calendar.detail.organizer": str("accounts", "Detail: organizer", "organizer"),
  "strings.calendar.detail.repeats": str("accounts", "Detail: repeats, rule unknown", "Repeats"),
  "strings.calendar.detail.you": str("accounts", "Detail: the user among the guests", "(you)"),
  "strings.calendar.rsvp.going": str("accounts", "Detail: the answer row", "Going?"),
  "strings.calendar.rsvp.yes": str("accounts", "Answer: yes", "Yes"),
  "strings.calendar.rsvp.maybe": str("accounts", "Answer: maybe", "Maybe"),
  "strings.calendar.rsvp.no": str("accounts", "Answer: no", "No"),
  "strings.calendar.form.bad_time": str(
    "accounts",
    "Form: a date or time is not valid",
    "Check the dates and times.",
  ),
  "strings.calendar.form.calendar": str("accounts", "Form: calendar", "Calendar"),
  "strings.calendar.form.custom_link": str("accounts", "Form: your meeting URL", "Meeting URL"),
  "strings.calendar.form.description": str("accounts", "Form: notes", "Notes"),
  "strings.calendar.form.start_date": str("accounts", "Form: start date", "Start date"),
  "strings.calendar.form.end_date": str("accounts", "Form: end date", "End date"),
  "strings.calendar.form.guests_placeholder": str(
    "accounts",
    "Form: guests placeholder",
    "Add guests by address",
  ),
  "strings.calendar.form.location": str("accounts", "Form: place", "Place"),
  "strings.calendar.form.meeting_link": str("accounts", "Form: meeting link", "Meeting link"),
  "strings.calendar.form.more": str("accounts", "Quick create: more options", "More options"),
  "strings.calendar.form.remove_guest": str("accounts", "Form: remove a guest", "Remove {name}"),
  "strings.calendar.form.repeat": str("accounts", "Form: repeat", "Repeat"),
  "strings.calendar.form.rule": str("accounts", "Form: custom rule", "Repeat rule"),
  "strings.calendar.form.time_zone": str("accounts", "Form: time zone", "Time zone"),
  "strings.calendar.form.title_placeholder": str(
    "accounts",
    "Form: title placeholder",
    "Add a title",
  ),
  "strings.calendar.link.default": str(
    "accounts",
    "Meeting link: the Setting's",
    "Meeting link from Settings",
  ),
  "strings.calendar.link.none": str("accounts", "Meeting link: none", "No meeting link"),
  "strings.calendar.link.meet": str("accounts", "Meeting link: Google Meet", "Google Meet"),
  "strings.calendar.link.teams": str("accounts", "Meeting link: Teams", "Microsoft Teams"),
  "strings.calendar.link.jitsi": str("accounts", "Meeting link: Jitsi", "Jitsi"),
  "strings.calendar.link.custom": str("accounts", "Meeting link: own URL", "My own link"),
  "strings.calendar.repeat.preset.none": str("accounts", "Repeat: never", "Does not repeat"),
  "strings.calendar.repeat.preset.daily": str("accounts", "Repeat: daily", "Every day"),
  "strings.calendar.repeat.preset.weekdays": str("accounts", "Repeat: weekdays", "Every weekday"),
  "strings.calendar.repeat.preset.weekly": str("accounts", "Repeat: weekly", "Every week"),
  "strings.calendar.repeat.preset.biweekly": str(
    "accounts",
    "Repeat: every two weeks",
    "Every two weeks",
  ),
  "strings.calendar.repeat.preset.monthly": str("accounts", "Repeat: monthly", "Every month"),
  "strings.calendar.repeat.preset.yearly": str("accounts", "Repeat: yearly", "Every year"),
  "strings.calendar.repeat.preset.custom": str("accounts", "Repeat: custom", "Custom"),
  "strings.calendar.repeat.custom": str(
    "accounts",
    "Rule in words: unreadable",
    "Repeats on a custom rule",
  ),
  "strings.calendar.repeat.daily": str("accounts", "Rule in words: daily", "Every day"),
  "strings.calendar.repeat.every_n_days": str(
    "accounts",
    "Rule in words: every n days",
    "Every {n} days",
  ),
  "strings.calendar.repeat.weekdays": str("accounts", "Rule in words: weekdays", "Every weekday"),
  "strings.calendar.repeat.weekly": str("accounts", "Rule in words: weekly", "Every week"),
  "strings.calendar.repeat.weekly_on": str(
    "accounts",
    "Rule in words: weekly on days",
    "Every week on {days}",
  ),
  "strings.calendar.repeat.every_n_weeks_on": str(
    "accounts",
    "Rule in words: every n weeks",
    "Every {n} weeks on {days}",
  ),
  "strings.calendar.repeat.monthly": str("accounts", "Rule in words: monthly", "Every month"),
  "strings.calendar.repeat.monthly_on": str(
    "accounts",
    "Rule in words: monthly on a day",
    "Every month on day {day}",
  ),
  "strings.calendar.repeat.yearly": str("accounts", "Rule in words: yearly", "Every year"),
  "strings.calendar.repeat.times": str("accounts", "Rule in words: a count", "{rule}, {n} times"),
  "strings.calendar.repeat.until": str("accounts", "Rule in words: an end", "{rule}, until {date}"),
  "strings.calendar.reminder.add": str("accounts", "Reminders: add", "Add reminder"),
  "strings.calendar.reminder.at_start": str("accounts", "Reminder: at the start", "At the start"),
  "strings.calendar.reminder.minutes": str("accounts", "Reminder: minutes", "{n} minutes before"),
  "strings.calendar.reminder.hours": str("accounts", "Reminder: hours", "{n} hours before"),
  "strings.calendar.reminder.days": str("accounts", "Reminder: days", "{n} days before"),
  "strings.calendar.reminder.default": str("accounts", "Reminder: the default", "Default, {when}"),
  "strings.calendar.reminder.none": str("accounts", "Reminder: none", "No reminder"),
  "strings.calendar.reminder.use_default": str(
    "accounts",
    "Reminders: back to the default",
    "Use the default",
  ),
  "strings.calendar.reminder.remove": str("accounts", "Reminders: remove one", "Remove {when}"),
  "strings.calendar.scope.edit_title": str(
    "accounts",
    "Recurring: change which",
    "Change a repeating Event",
  ),
  "strings.calendar.scope.delete_title": str(
    "accounts",
    "Recurring: delete which",
    "Delete a repeating Event",
  ),
  "strings.calendar.scope.this": str("accounts", "Recurring: this one", "This Event"),
  "strings.calendar.scope.following": str(
    "accounts",
    "Recurring: this and later",
    "This and following Events",
  ),
  "strings.calendar.scope.all": str("accounts", "Recurring: all", "All Events"),
  "strings.calendar.scope.ok": str("accounts", "Recurring: confirm", "OK"),
  "strings.calendar.send.invite_title": str("accounts", "Guests: invitations", "Send invitations?"),
  "strings.calendar.send.update_title": str(
    "accounts",
    "Guests: updates",
    "Send updates to the guests?",
  ),
  "strings.calendar.send.cancel_title": str(
    "accounts",
    "Guests: cancellations",
    "Send cancellations to the guests?",
  ),
  "strings.calendar.send.body": str(
    "accounts",
    "Guests: who gets mail",
    "{guests} will get an email about this.",
  ),
  "strings.calendar.send.more": str("accounts", "Guests: and more", "and {n} more"),
  "strings.calendar.send.confirm": str("accounts", "Guests: confirm", "Send"),
  "strings.calendar.search.label": str("accounts", "Search: label", "Search Events"),
  "strings.calendar.search.placeholder": str(
    "accounts",
    "Search: placeholder",
    "Search titles, places and people",
  ),
  "strings.calendar.search.close": str("accounts", "Search: close", "Close search"),
  "strings.calendar.search.none": str(
    "accounts",
    "Search: nothing found",
    "Nothing matches \u201c{query}\u201d",
  ),
  "strings.calendar.search.none_body": str(
    "accounts",
    "Search: nothing found, body",
    "Search looks through the Events this computer keeps: as far back as Calendar history and as far ahead as Calendar horizon in Settings.",
  ),
  "strings.calendar.problem.details": str("accounts", "Problem: the Provider's words", "Details"),
  "strings.calendar.problem.retry": str("accounts", "Problem: try again", "Try again"),
  "strings.calendar.problem.trying": str("accounts", "Problem: trying", "Trying"),
  "strings.calendar.problem.reconnect": str("accounts", "Problem: reconnect", "Sign in again"),
  "strings.calendar.problem.open_fix": str("accounts", "Problem: open the fix", "Open the fix"),
  "strings.calendar.problem.api_disabled.title": str(
    "accounts",
    "Problem: API off, title",
    "The calendar of {account} is turned off",
  ),
  "strings.calendar.problem.api_disabled.google": str(
    "accounts",
    "Problem: Google Calendar API off",
    "The Google Calendar API is not enabled in the Google Cloud project monday signs in with. Enable it for that project, wait a minute, then try again. Mail keeps working meanwhile.",
  ),
  "strings.calendar.problem.api_disabled.other": str(
    "accounts",
    "Problem: calendar API off elsewhere",
    "The calendar API is turned off for the app monday signs in with. Turn it on where the sign-in app is registered, then try again.",
  ),
  "strings.calendar.problem.scope.title": str(
    "accounts",
    "Problem: no calendar permission, title",
    "monday may not read the calendar of {account}",
  ),
  "strings.calendar.problem.scope.body": str(
    "accounts",
    "Problem: no calendar permission",
    "The sign-in did not allow the calendar. Sign in again and allow reading and changing your calendar.",
  ),
  "strings.calendar.problem.auth.title": str(
    "accounts",
    "Problem: sign-in, title",
    "The sign-in for {account} was refused",
  ),
  "strings.calendar.problem.auth.body": str(
    "accounts",
    "Problem: sign-in",
    "The sign-in expired or was withdrawn. Sign in again to read the calendar.",
  ),
  "strings.calendar.problem.network.title": str(
    "accounts",
    "Problem: offline, title",
    "Cannot reach the calendar of {account}",
  ),
  "strings.calendar.problem.network.body": str(
    "accounts",
    "Problem: offline",
    "The calendar server could not be reached. monday tries again on its own.",
  ),
  "strings.calendar.problem.rate_limit.title": str(
    "accounts",
    "Problem: quota, title",
    "The calendar of {account} is busy",
  ),
  "strings.calendar.problem.rate_limit.body": str(
    "accounts",
    "Problem: quota",
    "The provider asked monday to slow down. It tries again on its own in a few minutes.",
  ),
  "strings.calendar.problem.other.title": str(
    "accounts",
    "Problem: other, title",
    "The calendar of {account} could not be read",
  ),
  "strings.calendar.problem.other.body": str(
    "accounts",
    "Problem: other",
    "Something went wrong reading it. Try again, and see the details if it keeps happening.",
  ),
  "strings.palette.calendar_jump": str(
    "appearance",
    "Palette: jump to a date",
    "Calendar: go to {date}",
  ),
  "strings.settings.shortcuts.calendar": str("shortcuts", "Shortcut area: calendar", "Calendar"),
  "strings.action.calendar.today": str("shortcuts", "Action: calendar today", "Calendar: Today"),
  "strings.action.calendar.previous": str(
    "shortcuts",
    "Action: calendar previous",
    "Calendar: Previous",
  ),
  "strings.action.calendar.next": str("shortcuts", "Action: calendar next", "Calendar: Next"),
  "strings.action.calendar.view.day": str(
    "shortcuts",
    "Action: calendar view day",
    "Calendar: Day view",
  ),
  "strings.action.calendar.view.week": str(
    "shortcuts",
    "Action: calendar view week",
    "Calendar: Week view",
  ),
  "strings.action.calendar.view.month": str(
    "shortcuts",
    "Action: calendar view month",
    "Calendar: Month view",
  ),
  "strings.action.calendar.view.agenda": str(
    "shortcuts",
    "Action: calendar view agenda",
    "Calendar: Agenda view",
  ),
  "strings.action.calendar.new_event": str(
    "shortcuts",
    "Action: calendar new_event",
    "Calendar: New Event",
  ),
  "strings.action.calendar.search": str(
    "shortcuts",
    "Action: calendar search",
    "Calendar: Search Events",
  ),
  "strings.calendar.form.length_minutes": str("accounts", "End time: length in minutes", "{n} min"),
  "strings.calendar.form.length_hours": str("accounts", "End time: length in hours", "{h} h{m}"),
  "strings.calendar.form.add_guests": str("accounts", "Quick create: add guests", "Add guests"),
  "strings.calendar.form.description_placeholder": str(
    "accounts",
    "Editor: notes placeholder",
    "Add notes, an agenda or links",
  ),
  "strings.calendar.form.save_changes": str("accounts", "Editor: save an edited Event", "Save"),
  "strings.calendar.editor.when": str("accounts", "Editor section: when", "When"),
  "strings.calendar.editor.guests": str("accounts", "Editor section: guests", "Guests"),
  "strings.calendar.editor.guests_note": str(
    "accounts",
    "Editor: guests note",
    "They get an invitation when you save, after you say yes.",
  ),
  "strings.calendar.editor.no_guests": str(
    "accounts",
    "Editor: no guests",
    "Only you. Add people to invite them.",
  ),
  "strings.calendar.editor.notes": str("accounts", "Editor section: notes", "Notes"),
  "strings.calendar.editor.where": str("accounts", "Editor section: where", "Where"),
  "strings.calendar.editor.reminders": str("accounts", "Editor section: reminders", "Reminders"),
  "strings.calendar.draft.send_title": str(
    "accounts",
    "Draft: email the guests",
    "Apply and email the guests?",
  ),
  "strings.calendar.draft.moved": str("accounts", "Draft line: a moved Event", "{from}, now {to}"),
  "strings.calendar.draft.to_add": str("accounts", "Draft count: to add", "{n} to add"),
  "strings.calendar.draft.to_change": str("accounts", "Draft count: to change", "{n} to change"),
  "strings.calendar.draft.to_remove": str("accounts", "Draft count: to remove", "{n} to remove"),
  "strings.calendar.draft.more": str("accounts", "Draft card: more changes", "and {n} more"),
  "strings.calendar.draft.applied": str("accounts", "Draft: applied", "Applied"),
  "strings.calendar.draft.discarded": str("accounts", "Draft: discarded", "Discarded"),
  "strings.calendar.draft.show": str(
    "accounts",
    "Draft: show on the Calendar",
    "Show on the calendar",
  ),
  "strings.calendar.draft.discard": str("accounts", "Draft: discard", "Discard"),
  "strings.calendar.draft.apply_all": str("accounts", "Draft: apply all", "Apply all"),
  "strings.calendar.draft.apply_rest": str("accounts", "Draft: apply the rest", "Apply the rest"),
  "strings.calendar.draft.apply_some": str(
    "accounts",
    "Draft: apply the picked changes",
    "Apply {n}",
  ),
  "strings.calendar.draft.undo": str("accounts", "Draft: undo", "Undo"),
  "strings.calendar.draft.some_failed": str(
    "accounts",
    "Draft: some changes failed",
    "{n} could not be applied: {message}",
  ),
  "strings.calendar.draft.bar": str("accounts", "Draft bar: whose draft", "Draft from monday"),
  "strings.calendar.draft.review": str("accounts", "Draft bar: review", "Review"),
  "strings.calendar.draft.hide_review": str("accounts", "Draft bar: hide the list", "Hide list"),
  "strings.calendar.draft.done": str("accounts", "Draft toast: applied", "Applied {n} changes"),
  "strings.calendar.draft.waiting": str(
    "accounts",
    "Draft bar: drafts waiting",
    "{n} calendar drafts waiting",
  ),
  "strings.calendar.draft.open": str("accounts", "Draft bar: open a waiting draft", "Open"),
  "strings.calendar.draft.kind.create": str("accounts", "Draft change: add", "Add"),
  "strings.calendar.draft.kind.update": str("accounts", "Draft change: change", "Change"),
  "strings.calendar.draft.kind.delete": str("accounts", "Draft change: remove", "Remove"),
  "strings.calendar.draft.pick": str(
    "accounts",
    "Draft list: include a change",
    "Include this change",
  ),
  "strings.calendar.list.mine": str("accounts", "Calendar list: my calendars", "My calendars"),
  "strings.calendar.list.shared": str(
    "accounts",
    "Calendar list: shared with me",
    "Shared with me",
  ),
  "strings.calendar.list.only": str("accounts", "Calendar list: show only this", "Show only this"),
  "strings.calendar.list.hide": str("accounts", "Calendar list: hide", "Hide"),
  "strings.calendar.list.show": str("accounts", "Calendar list: show", "Show"),
  "strings.calendar.list.show_all": str("accounts", "Calendar list: show all", "Show all"),
  "strings.calendar.list.cant_read": str(
    "accounts",
    "Calendar list: cannot be read",
    "Cannot be read: {message}",
  ),
  "strings.calendar.list.free_busy": str("accounts", "Calendar list: busy times only", "busy only"),
  "strings.calendar.list.shared_by": str(
    "accounts",
    "Calendar list: shared by",
    "Shared by {name}",
  ),
  "strings.calendar.list.account_problem": str(
    "accounts",
    "Calendar list: account problem",
    "This account's calendar cannot be read. See the note above the calendar.",
  ),
  "strings.calendar.plan_week": str("accounts", "Header: plan my week", "Plan week"),
  "strings.calendar.plan_week_ask": str(
    "accounts",
    "Header: plan my week, the ask",
    "Plan my week: read my calendar for this week and my mail for tasks, deadlines and meetings people asked for, then propose focus blocks, those meetings and prep time as one calendar draft for me to review.",
  ),
  "strings.settings.calendar_shown.title": str(
    "appearance",
    "Settings: calendars per workspace, heading column",
    "Calendar",
  ),
  "strings.settings.calendar_shown.everywhere": str(
    "appearance",
    "Settings: calendars per workspace, all",
    "Every workspace",
  ),
  "strings.settings.calendar_shown.note": str(
    "appearance",
    "Settings: calendars per workspace, note",
    "A tick shows the calendar in that workspace's Calendar. The first column changes every workspace at once.",
  ),
  "strings.settings.calendar_shown.loading": str(
    "appearance",
    "Settings: calendars per workspace, loading",
    "Reading your calendars.",
  ),
  "strings.settings.calendar_shown.failed": str(
    "appearance",
    "Settings: calendars per workspace, failed",
    "Could not read the calendars: {message}",
  ),
  "strings.settings.calendar_shown.shared": str(
    "appearance",
    "Settings: calendars per workspace, shared tag",
    "shared",
  ),
  "strings.calendar.draft.include": str("accounts", "Draft change: keep it in", "Keep in"),
  "strings.calendar.draft.leave_out": str("accounts", "Draft change: leave it out", "Leave out"),
  ...
  "strings.compose.minimize": str("accounts", "Minimize compose", "Minimize"),
  "strings.compose.restore": str("accounts", "Restore a minimized message", "Open {subject}"),
  "strings.compose.close_draft": str(
    "accounts",
    "Close a minimized message, keeping the Draft",
    "Close, keep the Draft",
  ),
  "strings.compose.dock": str("accounts", "Dock of minimized messages", "Minimized messages"),
  "strings.compose.dock_more": str("accounts", "Folded minimized messages", "+{n}"),
  "strings.compose.unsaved": str("accounts", "Unsaved changes dot", "Unsaved changes"),
  "strings.compose.reply_on": str("accounts", "Dock chip for a reply", "Re: {subject}"),
  "strings.compose.no_recipient": str("accounts", "Dock chip without a recipient", "No recipient"),
  "strings.compose.discarded": str("accounts", "Draft discarded notice", "Draft discarded"),
  "strings.compose.drafted_by_agent": str(
    "accounts",
    "Byline on a Draft the Agent wrote",
    "Drafted by monday",
  ),
  "strings.compose.drafted_by_you": str("accounts", "Byline on a Draft you wrote", "Your draft"),
  "strings.compose.continue_draft": str("accounts", "Continue a saved Draft", "Continue"),
  "strings.compose.open_draft": str("accounts", "Open a Draft from an Agent card", "Open draft"),
  "strings.compose.clear_format": str("accounts", "Toolbar: clear formatting", "Clear formatting"),
  "strings.compose.assist": str("accounts", "Writing assist menu", "Assist"),
  "strings.compose.assist_on_selection": str(
    "accounts",
    "Assist menu heading with a selection",
    "Change the selection",
  ),
  "strings.compose.assist_on_body": str(
    "accounts",
    "Assist menu heading without a selection",
    "Change the message",
  ),
  "strings.compose.assist_shorter": str("accounts", "Assist: shorter", "Shorter"),
  "strings.compose.assist_clearer": str("accounts", "Assist: clearer", "Clearer"),
  "strings.compose.assist_friendlier": str("accounts", "Assist: friendlier", "Friendlier"),
  "strings.compose.assist_formal": str("accounts", "Assist: more formal", "More formal"),
  "strings.compose.assist_grammar": str("accounts", "Assist: fix grammar", "Fix grammar"),
  "strings.compose.assist_translate": str(
    "accounts",
    "Assist: translate",
    "Translate into {language}",
  ),
  "strings.compose.assist_continue": str(
    "accounts",
    "Assist: continue writing",
    "Continue writing",
  ),
  "strings.compose.assist_instruction": str(
    "accounts",
    "Assist: free instruction placeholder",
    "Tell monday what to change",
  ),
  "strings.compose.assist_voice": str("accounts", "Assist: in your voice note", "In your voice"),
  "strings.compose.assist_working": str("accounts", "Assist: waiting for the model", "Writing"),
  "strings.compose.assist_failed": str(
    "accounts",
    "Assist: the model call failed",
    "The assist did not answer: {error}",
  ),
  "strings.compose.assist_no_runtime": str(
    "accounts",
    "Assist: no runtime",
    "Set up a runtime in Settings to use the assist.",
  ),
  "strings.compose.suggestion": str("accounts", "Assist suggestion heading", "Suggestion"),
  "strings.compose.accept": str("accounts", "Accept a suggestion", "Accept"),
  "strings.compose.reject": str("accounts", "Reject a suggestion", "Reject"),
  ...
  "strings.action.thread.toggle_read": str(
    "shortcuts",
  ...
  "strings.action.compose.cycle": str(
    "shortcuts",
    "Action: cycle open and minimized messages",
    "Next draft",
  ),
  ...
  "strings.server.card.pick": str("server", "Cards: picker label", "Where the Cloud runs"),
  "strings.server.card.vercel.setup": str(
    "server",
    "Card: Vercel setup heading",
    "Deploy on Vercel",
  ),
  "strings.server.card.netlify.setup": str(
    "server",
    "Card: Netlify setup heading",
    "Deploy on Netlify",
  ),
  "strings.server.card.container.setup": str(
    "server",
    "Card: container setup heading",
    "Run it in your own container",
  ),
  "strings.server.card.optional": str("server", "Card: optional variable", "Optional"),
  "strings.server.card.copy": str("server", "Card: copy a variable name", "Copy {name}"),
  "strings.server.card.copied": str("server", "Card: copied", "Copied"),
  "strings.server.card.tag.push": str("server", "Card tag: push webhooks", "Instant push"),
  "strings.server.card.tag.closed": str(
    "server",
    "Card tag: sends while closed",
    "Sends while closed",
  ),
  "strings.server.card.tag.connections": str(
    "server",
    "Card tag: holds connections",
    "IMAP always on",
  ),
