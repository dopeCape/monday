...
The screen is a short title, one line, and three compact cards with one line each; "What's included" opens the longer account of each card below it.
...
Connecting the first Account, from the welcome's connect step or Settings › Accounts, leads to one screen that stands in for the whole app until the Inbox is fetched; so does any launch where the current Workspace's first sync has not finished. Nothing else is mounted behind it: no nav, no palette, no agent bar, no keymap. Two ways out are always there: the account header opens the workspace switcher (every Account, the current one checked, then Add an account, which goes through Settings › Accounts), and Settings opens the app's Settings with a way back to the screen. Picking another Account opens that Workspace, behind its own first sync screen if it has not finished. Once the wait is over it steps aside with one slow beat of the motion tokens (none with transitions off) and the app opens; the per-Account conversation offer follows as above. The screen shares the welcome's frame: the account and Settings along the top, the Provider's mark, the title and the phases centered in one panel.

- **Inbox ready**: once per Account, when its wait completes, monday says "Your inbox is ready" with the count synced ("32 emails synced for sam@monday.test."): a note at the bottom of the window while the window is in front (`notifications.note_ms`), a desktop notification otherwise, through the same path as calendar reminders. It watches every Account, so one that finishes while the user is in another Workspace, in Settings or in another app is still told, and so is one connected later whose small Inbox finished between two looks. An Account already there and already synced when the app starts is recorded without a word. Behind `notifications.inbox_ready` (on, and under `notifications.enabled`); what was told is kept in `sync.first_run_announced`.
...
- Settings: keymap (asks Vim, Gmail or Natural once; the conversation keeps the one chosen in the welcome unless the user names another), density from screen size, notification defaults.

## The browser demo

`bun scripts/demo.ts` (MONDAY_DEMO=1 on the Server, never the app) adds a scripted demo assistant under the Hosted runtime, so the whole flow, the conversation included, can be walked in the browser with no provider key: it holds a placeholder key for its provider, asks the questions in order, proposes Groups from the synced senders, sets the keymap and says it is done. A key the user pastes in the demo still reaches the real provider.
