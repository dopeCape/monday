...
Connecting the first Account, from the welcome's connect step or Settings › Accounts, leads to one screen that stands in for the whole app until the Inbox is fetched; so does any launch where the current Workspace's first sync has not finished. Nothing else is mounted behind it: no nav, no palette, no agent bar, no keymap. Once the wait is over it steps aside with one slow beat of the motion tokens (none with transitions off) and the app opens; the per-Account conversation offer follows as above.

- **Fetched** means, per the Setting `sync.first_run_wait`: `headers`, a mirror row for every Message the Provider holds in the Inbox (the Inbox's first paging finished); or `inbox_bodies` (the default), that and the body of every Inbox Message dated inside `sync.body_window_days`, so the first Threads opened render at once. Older bodies keep filling in after the app opens. The definition lives in `packages/shared/src/first-sync.ts`; the Server counts from `sync_messages` and its `body_state` (`GET /accounts/:id/sync`), and the engine fetches the Inbox's window bodies right after the Inbox's headers, before the other folders. Completion latches per Workspace, so later mail never brings the screen back.
...
- **On an error** (sign-in refused, Provider unreachable, anything else) it says why in plain words and offers the only two ways out: Retry, which clears the failure and syncs now, and Open settings, the Accounts section with a way back, where the Account can be reconnected or removed. A removed Account takes the app back to the Accounts screen.
