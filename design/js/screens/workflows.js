...
export function render(route, ui) {
  ...
  const card = w => `
    <div class="wf-card ${w.id === sel.id ? "on" : ""}" data-go="#/workflows/${w.id}">
      <div class="wf-top">
        <b>${w.name}</b>
        ${w.today ? `<span class="tag">${w.today} today</span>` : ""}${w.on ? "" : `<span class="tag">Paused</span>`}
    ...
    <div class="page-wrap">
      <div class="page-in">
        <div class="page-head">
          ...
          <div class="acts"><button class="btn outline">${ic("ph-clock-counter-clockwise")} Run history</button><button class="btn primary">${ic("ph-plus")} New workflow</button></div>
        </div>
        <div class="two">
          <div>
            ...
            <div class="empty page-empty">
              ...
              <div class="examples">
                <button>When a customer replies angry, draft an apology and flag it to me on Slack</button>
                <button>Every Monday, list open threads older than 5 days and snooze the rest</button>
                <button>When a calendar invite lands, check for conflicts and propose a new time</button>
                <button>Forward every invoice over 500 EUR to accounting with a summary</button>
              </div>
            </div>
          </div>
          <aside>
            <div class="side-card">
              <h3>${sel.name} <button class="btn sm">Source</button></h3>
              <div class="ask">${mark("sm")}<input placeholder="Ask monday to change this workflow" /></div>
